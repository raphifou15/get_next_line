 ...
 * @brief Fonction get_next_line_bonus
 */

/**
 * @brief Duplique une chaîne de caractères jusqu'au retour à la ligne.
 * 
 * La fonction `ft_strdup_until_carriage_return` copie les caractères d'une chaîne de caractères
 * `s` jusqu'à la rencontre du caractère de retour à la ligne (`\n`) ou de la fin
 * de la chaîne (`\0`). Elle renvoie une nouvelle chaîne de caractères contenant
 * la copie des caractères jusqu'au retour à la ligne.
 * 
 * @param[in] s La chaîne de caractères à dupliquer.
 * @return La nouvelle chaîne de caractères dupliquée, ou NULL en cas d'échec.
 */
...
/**
 * @brief Modifie une chaîne de caractères en enlevant une partie.
 * 
 * La fonction `ft_substr_modif` prend une chaîne de caractères `s`, un point de
 * départ `start`, une longueur `len`, un indice `i`, et renvoie une nouvelle
 * chaîne de caractères contenant une copie de `s` en commençant à `start`
 * et en s'arrêtant à `len` caractères (ou à la fin de la chaîne si `len`
 * dépasse la taille de la chaîne). De plus, elle libère la mémoire allouée
 * à la chaîne de caractères d'origine `s`.
 * 
 * @param[in] s La chaîne de caractères à modifier.
 * @param[in] start L'index du premier caractère à inclure dans la nouvelle chaîne.
 * @param[in] len Le nombre de caractères à inclure dans la nouvelle chaîne.
 * @param[in] i Un indice initial.
 * @return La nouvelle chaîne de caractères modifiée, ou NULL en cas d'échec.
 */
...
/**
 * @brief Extrait la première ligne d'une chaîne et met à jour le reste.
 * 
 * La fonction `ft_reste` extrait la première ligne d'une chaîne de caractères
 * `reste` et la stocke dans une variable pointée par `line`. Elle met ensuite
 * à jour la chaîne `reste` en supprimant la ligne extraite.
 * 
 * @param[in,out] reste La chaîne de caractères à modifier.
 * @param[out] line Pointeur vers la variable qui recevra la première ligne.
 * @return La chaîne de caractères `reste` mise à jour.
 */
 ...
 * 
 * La fonction `ft_free` libère la mémoire allouée à une chaîne de caractères
 * `s` et met le pointeur `s` à NULL.
 * 
 * @param[in,out] s La chaîne de caractères à libérer.
 * @return La chaîne de caractères `s` mise à NULL.
 */
char	*ft_free(char *s)
{
	free(s);
 ...
 * 
 * La fonction `get_next_line` lit une ligne depuis un descripteur de fichier
 * `fd` et la stocke dans une variable pointée par `line`.
 * 
 * @param[in] fd Le descripteur de fichier.
 * @param[out] line Pointeur vers la variable qui recevra la ligne lue.
